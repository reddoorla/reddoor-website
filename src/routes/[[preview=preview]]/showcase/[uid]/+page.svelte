	...
	<div class="w-full md:w-4/5">
		...
		<div class="w-full flex flex-col lg:flex-row mt-6 flex-wrap relative">
			...
			<a href={isFilled.link(pageData.projects[i].linkOverride) ? asLink(pageData.projects[i].linkOverride) : "/portfolio/"+project.uid} target={isFilled.link(pageData.projects[i].linkOverride) ? "_blank" : undefined} class="h-full w-full flex flex-col justify-end relative">
				<img src={pageData.projects[i].imageOverride.url||project.data.hero.url||''} alt={project.data.title  + " Hero Image"} class="absolute w-full h-full object-cover" loading="lazy"/>
				<div class="w-full h-full absolute top-0 left-0 hover:opacity-60 transition-opacity duration-700" style="background: linear-gradient(180deg, rgba(12, 19, 35, 0.15) 0%, rgba(12, 19, 35, 0.80) 81.09%) 50% / cover no-repeat;"></div>

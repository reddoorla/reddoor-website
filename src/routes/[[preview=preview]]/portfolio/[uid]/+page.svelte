        ...
        <div class=" text-white w-full z-10 mb-12 md:w-3/5 mx-auto">
            <div class="w-full border-b-1 border-b-white flex justify-between pb-4">
                ...
                <div class="h-6 w-20 flex flex-row justify-between text-white">
                    {#if data.prevProject}
                        <a href={"/portfolio/"+data.prevProject.uid} aria-label="Previous project"> <i class="fa-thin fa-sharp fa-chevron-left fa-2xl bump hover:text-light"></i></a>
                    {/if}
                    {#if data.nextProject}
                        <a href={"/portfolio/"+data.nextProject.uid} aria-label="Next project"> <i class="fa-thin fa-sharp fa-chevron-right fa-2xl bump hover:text-light"></i></a>
                    {/if}
                </div>
            </div>
    ...
    <div class="w-full mt-12 flex flex-col md:flex-row gap-12 md:gap-0">
      ...
      <div class="w-full md:w-2/5 md:pr-2 aspect-4/3 relative">
        <a href={"/portfolio/"+(data.relatedProjectOne.uid||'')} class="h-full w-full flex flex-col justify-end relative">
          <img src={data.relatedProjectOne.data.hero.url||''} alt={(data.relatedProjectOne.data.title||'') + " Hero Image"} class="absolute w-full h-full object-cover" loading="lazy"/>
      ...
      <div class="w-full md:w-2/5 md:pl-2 aspect-4/3 relative">
        <a href={"/portfolio/"+data.relatedProjectTwo.uid} class="h-full w-full flex flex-col justify-end relative">
          <img src={data.relatedProjectTwo.data.hero.url||''} alt={(data.relatedProjectTwo.data.title||'') + " Hero Image"} class="absolute w-full h-full object-cover" loading="lazy"/>
          <div class="w-full h-full absolute top-0 left-0 hover:opacity-60 transition-opacity duration-700" style="background: linear-gradient(180deg, rgba(12, 19, 35, 0.15) 0%, rgba(12, 19, 35, 0.80) 81.09%) 50% / cover no-repeat;"></div>
          <div class="w-full flex flex-row justify-between p-6 z-10" >
              <div>

...
<!-- Narrowed inline (not via a $derived boolean) so `slice` is typed as the
     matching variation inside each branch. -->
{#if slice.variation === "expandable"}
  <!-- Landing-page band, in its own component so the default variation's markup
       and scoped styles below are byte-for-byte untouched (the `svelte-…` scope
       class is hashed from this file's CSS text). -->
  <Expandable {slice} />
{:else}
  <SliceSection {slice} class="w-screen py-12 bg-paper-red relative">
    <ContentWidth>

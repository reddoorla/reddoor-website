<script lang="ts">
  ...
  const handleScroll = () => {
    ...
    const pct = Math.min(
      Math.max(
        100 - ((containerRect.bottom - viewportHeight) / (containerRect.height - viewportHeight)) * 100,
        0
      ),
      100
    );
 ...
 {/if}
     ...
     <div class="w-screen h-lvh fixed bg-paper-red top-0 left-0 z-50" transition:fly={{y:"-100%"}}>
      <ContentWidth class="h-full flex flex-col items-center justify-center gap-12 relative">
        <a href="/" class="text-white text-2xl" onclick={()=>isOverlayVisible=false}>Home</a>
        <a href="/portfolio" class="text-white text-2xl" onclick={()=>isOverlayVisible=false}>Portfolio</a>
        <a href="/about" class="text-white text-2xl" onclick={()=>isOverlayVisible=false}>About</a>
        <a href="/twenty-for-twenty" class="text-white text-2xl" onclick={()=>isOverlayVisible=false}>20 for 20</a>
        <a href="/contact" class="text-white text-2xl" onclick={()=>isOverlayVisible=false}>Meet With Us</a>
        <button class="absolute top-12 right-0 opacity-60 hover:opacity-100 transition z-40" onclick={()=>isOverlayVisible=false} aria-label="Close menu">
 ...
 <div class="w-screen" bind:this={openingSection}>
   <div class="h-dvh w-screen fixed bottom-0 left-0 bg-paper-red">
     ...
     <ContentWidth class="flex flex-col justify-center items-center h-full z-10 relative {percentageScrolled > 30 ? "opacity-0 ": 'opacity-100'}">
       ...
       <i class="fa-light fa-arrow-down fa-2xl opacity-50 absolute bottom-12 text-white bob-always"></i>
     </ContentWidth>
     ...
     <div class="fixed top-0 left-0 w-screen h-dvh overflow-hidden z-20">
      <ContentWidth class="h-32 flex justify-between items-center">
        ...
        <button class="opacity-100 hover:opacity-60 transition " onclick={() => isOverlayVisible = true} in:fade={{delay:400}} out:fade aria-label="Open menu">

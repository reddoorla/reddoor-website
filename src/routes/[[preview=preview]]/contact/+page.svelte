<script lang="ts">
    ...
    const handleSubmit = async (event: SubmitEvent) => {
        ...
        try {
            ...
            submitError = "Something went wrong sending your message. Please try again or email info@reddoorla.com.";
        } finally {
            submitting = false;
        }
    };
...
{#if submitted}
<button class="h-screen w-screen fixed bg-black/20 z-50 flex justify-center items-center cursor-pointer" onclick={()=>submitted=false} transition:fade aria-label="Close thank-you message">
    <div class="w-[68vw] h-[68vh] bg-paper-red flex items-center justify-center p-32 relative">
        <i class="fa-solid fa-thin fa-close fa-2xl text-white hover:text-light transition absolute top-8 right-5" aria-hidden="true"></i>
...
<div class="w-screen bg-white py-12">
    ...
    <ContentWidth class="flex flex-col md:flex-row mb-48" animateIn>
        ...
        <div class="w-full md:w-4/5 flex flex-col gap-8">
            ...
            <form class="h-full w-full mt-8 md:mt-0 md:w-2/3 flex flex-col gap-2 items-start md:pr-24" name="contact" method="POST" onsubmit={handleSubmit} data-netlify="true" data-netlify-honeypot="bot-field">
                ...
                <input type="hidden" name="form-name" value="contact" />
                    <AnimateIn class="w-full">
                        ...
                        <p class="hidden">
                          ...
                          <AnimateIn class="w-full">
                        <p>Message</p>
                        <textarea name="message" required placeholder="how can we help?" class="min-h-24 w-full border-1 border-mid p-1 mb-4"></textarea>
                    </AnimateIn>
                    <AnimateIn>
                        <input
                          type="submit"

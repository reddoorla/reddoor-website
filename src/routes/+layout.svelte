...
<div class="w-screen h-lvh fixed bg-paper-red flex flex-col items-center justify-center gap-12 z-30" transition:fly={{y:"-100%"}}>
    ...
    {#each NAV_LINKS as item (item.href)}
        <a href={item.href} class="text-white text-2xl" onclick={()=>isOverlayVisible=false}>{item.label}</a>
    {/each}

    <button class="absolute top-5 right-5 opacity-60 hover:opacity-100 transition z-40" onclick={toggleOverlayOff} aria-label="Close menu">
...
<main out:fade={{duration:500}} in:fade={{delay:700, duration:700}}>
            ...
            <div class="flex flex-row">
                <div class="hidden lg:flex flex-row justify-between items-center gap-10">
                    {#each NAV_LINKS as item (item.href)}
                        ...
                        <a class="label hover:opacity-80 transition-opacity " href={item.href} onclick={()=>isOverlayVisible=false}>{item.label}</a>

                    {/each}
                </div>

            <button class="lg:hidden ml-6 opacity-60 hover:opacity-100 transition-all" onclick={toggleOverlayOn} aria-label="Open menu">
               {#if !isOverlayVisible}
	...
	<!-- nav #2 -->
{#if showNav &&!isTop.value}
<div class="h-12 w-screen top-0 fixed z-20 bg-white/80" transition:fly={{y:-64, delay:500}}>
    <ContentWidth class="flex flex-row justify-between items-center h-full">
        <a href="/" class="hover:opacity-80 transition-all duration-500 bump label">
            Reddoor Creative
        </a>
        ...
        <div class="flex flex-row">
            <div class="hidden lg:flex flex-row justify-between items-center gap-10">
                {#each NAV_LINKS as item, i (item.href)}
                    ...
                    <a class="label" href={item.href} onclick={()=>isOverlayVisible=false}>{item.label}</a>
                    {:else}
                    <a class="text-primary label" href={item.href} onclick={()=>isOverlayVisible=false}>{item.label}</a>
                    {/if}
                {/each}
            </div>

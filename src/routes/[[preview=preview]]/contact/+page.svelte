...
{#if submitted}
<button class="h-screen w-screen fixed bg-black bg-opacity-20 z-50 flex justify-center items-center" on:click={()=>submitted=false} transition:fade>
...
<div class="w-screen bg-white py-12">
    ...
    <ContentWidth class="flex flex-col md:flex-row mb-48">
        ...
        <div class="w-full md:w-4/5 flex flex-col gap-8">
            ...
            <form class="w-full h-full flex flex-col md:flex-row px-8"  name="contact" method="POST" on:submit={handleSubmit} data-netlify="true" data-netlify-honeypot="bot-field" on:submit={()=>submitted=true}>
